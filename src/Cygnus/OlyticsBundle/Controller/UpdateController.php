...
class UpdateController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        foreach ($groups as $group) {
            ...
            for ($i = 0; $i < $pages; $i++) {
                ...
                $ts = ($lastInsert === null) ? 0 : $lastInsert->sec;
                echo sprintf("Requesting %s ad impressions since %s from %s::%s on %s\r\n", number_format($this->getSourceRecordLimit($recordLimit)), date('Y-m-d H:i:s', $ts), $this->getOlyticsDatabase($group), $this->getOlyticsColl(), $this->getOlyticsConnName($group));
                ...
                echo sprintf('Data will be aggregated to %s::%s on %s'."\r\n", $this->getFeedDb($group), $this->getFeedCollection($group), $this->getFeedConnName());
    ...
    public function getAdEventsByDate($group, \MongoDate $lastInsert = null, $recordLimit = null)
    {
        ...
        $builder = $this->createOlyticsQueryBuilder($group, $lastInsert)
            ->find()
        ...
        if ($ts === 0) {
            ...
            $stop = strtotime('+3 Months', $lastInsert->sec);

            $builder
                ->field('createdAt')->gt($lastInsert)->lt(new \MongoDate($stop))
        ...
        if ($lastInsert === null) {
            die('NULL LAST INSERT. MUST IMPLEMENT.');
        }
        $maxRange = '+10 Days';
        $stop = strtotime($maxRange, $lastInsert->sec);
    ...
    protected function getOlyticsDatabase($group)
    {
        ...
        return sprintf('oly_%s_%s_events', $account, $group);
    }

    protected function getOlyticsColl()
    {
        return 'ad';
    }
    ...
    protected function createOlyticsQueryBuilder($group, \MongoDate $lastInsert = null)
    {
        $db = $this->getOlyticsDatabase($group);
        $coll = $this->getOlyticsColl();
    ...
    {
        $account = (in_array($group, ['fcp', 'fl', 'ooh', 'sdce'])) ? 'acbm' : 'cygnus';
        return sprintf('%s_%s', $account, $group);
    }
